...
public final class NSALoginController {
    ...
    public static void hashUserPassword(User user) throws Exception {
        ...
        // Throw weak password exception if minimum requirements not met
        if (!checkComplexity(user.getPassword())) {
    ...
    // Checks password complexity requirements throwing an exception on failure
    // and returns true if there is a match
    private static Boolean checkComplexity(String password) {
        // Matches for at least 1 number, no white spaces, and at least 8 characters
